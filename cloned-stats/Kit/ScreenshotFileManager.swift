...
public class ScreenshotFileManager {
    ...
    public static let shared = ScreenshotFileManager()
    ...
    public func saveScreenshot(_ image: NSImage) -> URL? {
    ...
    public func getCurrentSessionFolder() -> URL {
        var result: URL!
    ...
    /// FIX: Use async dispatch to avoid deadlock when called from within queue context
    public func getCurrentSessionNumber() -> Int {
        var result: Int = 0
        let semaphore = DispatchSemaphore(value: 0)
        queue.async {
    ...
    /// FIX: Use async dispatch to avoid deadlock when called from within queue context
    public func getCurrentSessionCount() -> Int {
        var result: Int = 0
        let semaphore = DispatchSemaphore(value: 0)
        queue.async {
    ...
    public func createNewSessionIfNeeded() {
    ...
    public func getScreenshotCount(inSession session: Int) -> Int {
    ...
    public func getAllSessions() -> [URL] {
        do {
            let contents = try FileManager.default.contentsOfDirectory(
                at: baseDirectory,
    ...
    public func getScreenshots(inSession sessionNumber: Int) -> [URL] {
