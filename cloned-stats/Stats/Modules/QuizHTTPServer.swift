...
struct HTTPResponse {
    ...
    init(statusCode: Int, body: String, headers: [String: String] = [:]) {
        ...
        self.body = body
        // Merge dictionaries properly in Swift
...
class HTTPServer: NSObject, StreamDelegate {
    ...
    func start() -> Bool {
        ...
        listeningSocket = CFSocketCreate(
            ...
            { socket, callbackType, address, data, info in
                ...
                if callbackType == .acceptCallBack {
                    if let data = data {
                        // CRASH FIX: data is an UnsafeRawPointer to CFSocketNativeHandle
                        // Safely bind and read the socket handle
                        let nativeSocket = data.load(as: CFSocketNativeHandle.self)

                        guard nativeSocket > 0 else {
                            print("❌ Invalid native socket handle: \(nativeSocket)")
                            return
                        }
        ...
        // Use Darwin's in_addr_t for INADDR_LOOPBACK (127.0.0.1 in network byte order)
    ...
    private func handleConnection(_ nativeSocket: CFSocketNativeHandle) {
        queue.async { [weak self] in
            ...
            do {
                ...
                // CRASH FIX: Safe unwrap instead of force unwrap
                guard let responseData = responseString.data(using: .utf8) else {
                    print("❌ Failed to encode response string to UTF-8")
                    fileHandle.closeFile()
                    return
                }

                fileHandle.write(responseData)
                fileHandle.closeFile()
